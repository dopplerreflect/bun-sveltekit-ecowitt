...
import parse, { type Rap } from "gsl-parser";
// import { getTrimmedCoordinates } from "../helpers/coordinates";
import type { WindsAloftReport } from "../../ppg.report/WindsAloft";
import { fetchWindsAloftForecasts } from "../winds-aloft/fetchWindsAloftForecasts";
...
async function _getRap(
  ...
  // Axios was returning an error about non escaped characters and I knew
  // I already had working code for this, so...
  ...
  // here's that - doppler
  const asciiReports = await fetchWindsAloftForecasts(
    lat.toString(),
    lon.toString(),
  );
...
function getTrimmedCoordinates(lat: number, lon: number): string {
